...
const argv = minimist(process.argv.slice(2), {
  string: ['l'],
  boolean: ['cache', 'd', 'error-key', 'h', 'a', 'g'],
  default: {
    'cache': true,
    'd': false,
    'h': false,
...
const options = argvFromCache(argv)
const pretty = Pretty({
...
// read arguments from cache './mprjc'
function argvFromCache (argv) {
  if (!argv.cache) return argv
  let cache
  if ((process.argv.length - argv._.length) < 3 ) {
    try {
      cache = fs.readFileSync('./.mpjrc', 'utf8')
      return cache ? JSON.parse(cache) : argv
    } catch (err) {
      return argv
    }
  } else {
    const _ = argv._
    delete argv._
    fs.writeFile('./.mpjrc', JSON.stringify(argv, null, 2), (err) => {
      if (err) console.error('[munia-pretty-json] Could not write cache: ',
        err.message);
    })
    argv._ = _
    return argv
